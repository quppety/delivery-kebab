...
async function init() {
  ...
  if (adressClient !== null) {
    ...
    const geocodePromises = addressProduct.map((el) => {
      return new Promise((resolve) => {
        setTimeout(async () => {
          try {
            const resultB = await ymaps.geocode(el.curr_location);
            var pointB = resultB.geoObjects.get(0).geometry.getCoordinates();
            var distance = ymaps.coordSystem.geo.getDistance(pointA, pointB);
            el.distance = distance;
            console.log('Расстояние между точками:', distance, 'м');
          } catch (error) {
            console.error('Произошла ошибка при вычислении расстояния:', error);
          }
          resolve();
        }, 100);
      });
    ...
    container.innerHTML = adressProduct.map(
      (el) => `
